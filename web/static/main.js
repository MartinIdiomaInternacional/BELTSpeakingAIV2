/* BELT Voice Evaluator - Main JS (with debug) */
...
let mediaStream=null, mediaRecorder=null, recordedChunks=[], isRecording=false, timerInterval=null, startTimestamp=null;
...
function appendMessage(msg,type="info"){ const el=$(SEL.messages); if(!el)return; const d=document.createElement("div"); d.className=`msg ${type}`; d.textContent=msg; el.prepend(d); }

function renderPrompt(p,l){ setText(SEL.levelBadge,l||""); setText(SEL.promptText,p||STR.defaultPrompt); }
function renderAttempt(n){ setText(SEL.attemptBadge, n?`Attempt ${n}`:""); }

function chip(label,val){ const v=Math.round((val??0)*100); let color=SCORE_COLORS.low; if(v>=PASS_MIN_THRESHOLD*100&&v<PASS_AVG_THRESHOLD*100) color=SCORE_COLORS.mid; if(v>=PASS_AVG_THRESHOLD*100) color=SCORE_COLORS.good; return `<div class="score-chip" style="border:1px solid #e5e7eb;border-radius:999px;padding:6px 10px;display:inline-flex;align-items:center;gap:8px;margin:4px;"><span style="font-weight:600">${label}</span><span style="background:${color};color:#fff;border-radius:999px;padding:2px 8px;">${v}</span></div>`; }

function renderScores(r){ const b=$(SEL.scoresContainer); if(!b)return; if(!r||!r.scores){ setHTML(SEL.scoresContainer,""); return; } const s=r.scores, avg=r.average??0; const html=`<div>${chip("Fluency",s.fluency)}${chip("Grammar",s.grammar)}${chip("Vocabulary",s.vocabulary)}${chip("Pronunciation",s.pronunciation)}${chip("Coherence",s.coherence)}</div><div style="margin-top:6px"><strong>Average:</strong> ${(avg*100).toFixed(0)} ${r.decision?`<span style="margin-left:8px;padding:2px 8px;border-radius:6px;background:${r.decision==='advance'?'#d1fae5':'#fee2e2'};color:${r.decision==='advance'?'#064e3b':'#7f1d1d'}">${r.decision}</span>`:""}</div>`; setHTML(SEL.scoresContainer,html); }
function renderRecommendations(recs){ if(!recs||!recs.length){ setHTML(SEL.recsContainer,""); return; } setHTML(SEL.recsContainer, `<ul>${recs.map(r=>`<li>${r}</li>`).join("")}</ul>`); }
function renderFinalReportLink(id){ const el=$(SEL.finalReport); if(!el)return; el.innerHTML=`<a href="${API_BASE}/report/${encodeURIComponent(id)}" target="_blank" rel="noopener">Open Final Report</a>`; }
...
function startTimer(){ resetProgressUI(); startTimestamp=Date.now(); timerInterval=setInterval(()=>{ const elapsed=Math.floor((Date.now()-startTimestamp)/1000); const pct=Math.min(100,Math.floor((elapsed/RECORD_SECONDS)*100)); const bar=$(SEL.progressBar),t=$(SEL.timerText); if(bar) bar.style.width=`${pct}%`; if(t) t.textContent=`${Math.min(elapsed,RECORD_SECONDS)}/${RECORD_SECONDS}s`; if(elapsed>=RECORD_SECONDS) stopRecordingAndSend(); },250); }
...
async function startRecording(){
  ...
  catch(e){ appendMessage("Mic permission denied or unavailable.","error"); return; }
  recordedChunks=[];
  try{
    mediaRecorder=new MediaRecorder(mediaStream,{mimeType:"audio/webm;codecs=opus"});
  }catch(e){
    mediaRecorder=new MediaRecorder(mediaStream); // fallback, browser decides
  }
...
async function startSession(){
  try{
    ...
    setDisabled(SEL.btnRetry,false); setDisabled(SEL.btnStart,false); setDisabled(SEL.btnStopSend,true);
...
async function fetchPromptForLevel(level){
  ...
  CURRENT_PROMPT_ID=typeof data.prompt_id==="number"?data.prompt_id:null;
  renderPrompt(CURRENT_PROMPT,level); resetScoresUI(); resetProgressUI();
...
async function submitRecording(blob){
  try{
    ...
    if(CURRENT_PROMPT_ID!==null){ fd.append("prompt_id",String(CURRENT_PROMPT_ID)); } else { fd.append("question",CURRENT_PROMPT); }
...
async function init(){
  ...
  // Mirror JS errors into the page for quick diagnosis

...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Adjust to your GitHub Pages domain for better security
...
# ==============================
# Question bank per CEFR level (20 each)
# ==============================
questions = {
    "A1": [
...
def analyze_text(transcript):
    ...
    Provide CEFR level for each and list 1-2 specific mistakes found.
    Transcript:
    \"\"\"{transcript}\"\"\"
...
async def start_test():
    ...
    sessions[session_id] = {"questions_asked": 0, "answers": [], "current_level": "A1", "asked_questions": set()}
...
async def next_question(session_id: str = Form(...), file: UploadFile = File(...)):
    try:
        ...
        current_index = level_order.index(sessions[session_id]["current_level"])

        if pron_level in ["B2","C1","C2"] or "B2" in text_eval or "C" in text_eval:
            current_index = min(current_index+1, 5)

        next_level = level_order[current_index]
        sessions[session_id]["current_level"] = next_level

        # Avoid repeating questions
        available_questions = list(set(questions[next_level]) - sessions[session_id]["asked_questions"])
        if not available_questions:
            available_questions = questions[next_level]
            sessions[session_id]["asked_questions"] = set()

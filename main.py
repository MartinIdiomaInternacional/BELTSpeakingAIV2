...
APP_VERSION = "BELT Speaking Test API v0.6 (2025-08-14)"
...
def load_prompts():
    ...
    try:
        ...
        # Normalize keys (upper-case, strip)
        norm = {str(k).strip().upper(): v for k, v in data.items()}
        return norm
    except Exception:
        return {}
...
def get_ssl_embedding(y_16k: np.ndarray):
    ...
    import torch  # heavy imports only in FULL mode
    import torchaudio
...
async def evaluate(file: UploadFile = File(...)):
    # Requires python-multipart; use /evaluate-bytes if you can't install it.
...
async def evaluate_bytes(request: Request):
    """Alternate endpoint that accepts the raw audio in the request body.
    Use Content-Type like audio/wav, audio/mpeg, application/octet-stream.
    Does NOT require python-multipart.
    """
    try:
